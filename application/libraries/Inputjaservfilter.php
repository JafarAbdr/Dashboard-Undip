...
class Inputjaservfilter {
	...
	public function nameFacultyFiltering($faculty){
		$faculty.="";
		$faculty = $this->stringFiltering($faculty);
		if(($faculty == "") || ($faculty == " "))
			return (array(false,"tidak boleh kosong"));
		if(!preg_match("/^[a-zA-Z& ]*$/",$faculty))
			return (array(false,"faculty mengandung karakter yang tidak diizinkan"));
		return (array(true,"valid"));
	}
	public function jenjangFiltering($jenjang){
		$jenjang.="";
		$jenjang = $this->stringFiltering($jenjang);
		if(($jenjang == "") || ($jenjang == " "))
			return (array(false,"tidak boleh kosong"));
		if(!preg_match("/^[a-zA-Z0-9 ]*$/",$jenjang))
			return (array(false,"jenjang mengandung karakter yang tidak diizinkan"));
		return (array(true,"valid"));
	}
	/*
	test : true
